...
@Autowired
	...
	Session session=sessionFactory.getCurrentSession();
		SQLQuery sqlQuery=session.createSQLQuery("select * from usertbl where email in (select email from usertbl where email!=? minus (select fromId from friendtbl where toId=? union select toId from friendtbl where fromId=?))");
	...
	public List<User> listOfFriends(String email) {
		...
		Session session=sessionFactory.getCurrentSession();
		SQLQuery query=
		session.createSQLQuery("select * from usertbl where email in"
				+ "(select fromId from friendtbl where toId=? and status='A' "
				+ " union "
				+ "select toId from friendtbl where fromId=? and status='A')");
		query.setString(0, email);
